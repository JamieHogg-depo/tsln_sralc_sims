...
# Check Rhat's
s1LN_btess <- suppressMessages(get_bulktailESS(s1LN_fit, chains = 1:n.ch))
...
message(paste0("SR: ", round(median(s1LN_its$SR), 2)))
...
s1LN_its$mu_pd <- cbind(s1LN_its$mu_pd, matrix(NA, ncol = 40, nrow = nrow(s1LN_its$mu_pd)))
